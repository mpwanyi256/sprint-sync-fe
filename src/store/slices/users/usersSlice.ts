...
export const fetchUsers = createAsyncThunk<UsersResponse, { page: number; limit: number; search?: string }>(
  'users/fetchUsers',
  async ({ page, limit, search = '' }) => {
    const searchParam = search.trim() ? `&search=${encodeURIComponent(search)}` : ''
    const response = await api.get(`/users/?page=${page}&limit=${limit}${searchParam}`)
