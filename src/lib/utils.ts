...
export const findTaskInColumns = (columns: any, taskId: string): { task: Task | null; columnStatus: TaskStatus | null; taskIndex: number } => {
  for (const columnStatus of Object.keys(columns)) {
    ...
    if (taskIndex !== -1) {
      return { task: column.tasks[taskIndex], columnStatus: statusKey, taskIndex }
    }
  }
  return { task: null, columnStatus: null, taskIndex: -1 }
}

export const removeTaskFromColumn = (columns: TaskState['columns'], columnStatus: TaskStatus, taskIndex: number) => {
...
export const addTaskToColumn = (columns: TaskState['columns'], columnStatus: TaskStatus, task: Task) => {
