...
const TaskListView = ({ onViewTaskDetails }: TaskListViewProps) => {
  ...
  const todoPagination = useAppSelector(state => selectColumnPagination(state, 'TODO'))
  const inProgressPagination = useAppSelector(state => selectColumnPagination(state, 'IN_PROGRESS'))
  const donePagination = useAppSelector(state => selectColumnPagination(state, 'DONE'))
  ...
  const loadMore = useCallback(async (status: TaskStatus) => {
    ...
    setLoading(true)
    try {
      const pagination = status === 'TODO' ? todoPagination : 
                        status === 'IN_PROGRESS' ? inProgressPagination : donePagination
      
      if (pagination.hasNextPage) {
        await dispatch(fetchTasks({ status, page: pagination.currentPage + 1 }))
      }
    } catch (error) {
    ...
    return (
      <div 
        ...
        className={cn(
          ...
          isDragOver ? "border-blue-400 bg-blue-50/50 shadow-lg" : "border-gray-200 shadow-sm"
        )}
