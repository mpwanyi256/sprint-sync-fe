...
export const TaskDetailsContent = ({
  ...
  const { moveTask, updateDetails } = useOptimisticTaskUpdates();
  ...
  const handleSaveTask = async () => {
    ...
    try {
      const updatedTask = await updateDetails(localTask.id, {
        title,
        description,
  ...
  return (
    <>
      <div className='flex items-start gap-3 border-b border-gray-200 p-2'>
        <DialogTitle
          ref={titleRef}
          contentEditable
