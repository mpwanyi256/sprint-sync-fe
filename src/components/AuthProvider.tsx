...
import { fetchCurrentUser, selectUser, selectIsAuthenticated, setAuthenticated } from '@/store/slices/auth'
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    console.log('AuthProvider - Navigation effect:', { isAuthenticated, user, pathname })
    
    if (isAuthenticated && user && pathname === '/login') {
      // User is authenticated, redirect to dashboard
      console.log('AuthProvider - User is authenticated, redirecting to dashboard')
      router.push('/')
